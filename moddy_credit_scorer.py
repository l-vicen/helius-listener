...
def derivingVariableY(dataframe, balances, unique_addresses, beforeAndAfterNativeBalances):
    ...
    # calculates the slope rate by dividing the number of transactions to the diffrence of balance
    adj_vector = [x / y for x, y in zip(payouts, transactions_per_payer)]
    st.write("Adjusted by number transactions")
    st.write(adj_vector)
    
    # scales the result down by 50% due to scaling reasons
    adj_vector_div_half = [x / 2 for x in adj_vector]
    st.write("Scaling")
    st.write(adj_vector_div_half)
    
    # feeds the slope rate x into the function arctan that is slightly shifted to the left of the y coordinate
    trig_behavior = [((np.arctan(x) / (0.65)) + 0.05) for x in adj_vector_div_half]
    st.write("Trigonometric Adjustment")
    st.write(trig_behavior)
    
    # if y is smaller than 0, than the function simply returns 0 since that will be out of the range
    # otherwise the value of y will be returned
...
def calculateCreditScore(variable_x, variable_y, variable_z):
    return np.multiply.reduce((variable_x, variable_y, variable_z))   
